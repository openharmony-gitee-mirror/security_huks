...
namespace HuksNapi {
namespace {
constexpr int HUKS_NAPI_UNWRAP_KEY_MIN_ARGS = 3;
constexpr int HUKS_NAPI_UNWRAP_KEY_MAX_ARGS = 4;

constexpr int HKS_MAX_WRAPPED_DATA_SIZE = 2048;
}  // namespace

struct WrapKeyAsyncContext_t {
...
using WrapKeyAsyncContext = WrapKeyAsyncContext_t *;

static WrapKeyAsyncContext CreateWrapKeyAsyncContext()
{
    WrapKeyAsyncContext context = (WrapKeyAsyncContext)HksMalloc(sizeof(WrapKeyAsyncContext_t));
    if (context != nullptr) {
        (void)memset_s(context, sizeof(WrapKeyAsyncContext_t), 0, sizeof(WrapKeyAsyncContext_t));
    }
    return context;
}
...
{
    size_t argc = HUKS_NAPI_UNWRAP_KEY_MAX_ARGS;
    napi_value argv[HUKS_NAPI_UNWRAP_KEY_MAX_ARGS] = {0};
    NAPI_CALL(env, napi_get_cb_info(env, info, &argc, argv, nullptr, nullptr));

    if (argc < HUKS_NAPI_UNWRAP_KEY_MIN_ARGS) {
...
static napi_value WrapKeyWriteResult(napi_env env, WrapKeyAsyncContext context)
{
    return GenerateHksResult(env,
        context->result,
        ((context->result == HKS_SUCCESS && context->wrappedData != nullptr) ? context->wrappedData->data : nullptr),
        (context->result == HKS_SUCCESS && context->wrappedData != nullptr) ? context->wrappedData->size : 0);
}
...
static napi_value WrapKeyAsyncWork(napi_env env, WrapKeyAsyncContext context)
{
    ...
    napi_create_string_latin1(env, "wrapKeyAsyncWork", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(
        ...
        [](napi_env env, void *data) {
            ...
            if (context->wrappedData != nullptr) {
                context->wrappedData->data = (uint8_t *)HksMalloc(HKS_MAX_WRAPPED_DATA_SIZE);
                context->wrappedData->size = HKS_MAX_WRAPPED_DATA_SIZE;
            }

            context->result =
                HksWrapKey(context->keyAlias, context->targetKeyAlias, context->paramSet, context->wrappedData);
        },
