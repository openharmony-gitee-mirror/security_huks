...
namespace HuksNapi {
namespace {
constexpr int HUKS_NAPI_DERIVE_KEY_MIN_ARGS = 2;
constexpr int HUKS_NAPI_DERIVE_KEY_MAX_ARGS = 3;
...
static void DeleteDeriveKeyAsyncContext(napi_env env, DeriveKeyAsyncContext context)
{
    ...
    if (context->derivedKey != nullptr) {
        if (context->derivedKey->data != nullptr && context->derivedKey->size != 0) {
            (void)memset_s(context->derivedKey->data, context->derivedKey->size, 0, context->derivedKey->size);
        }
        FreeHksBlob(context->derivedKey);
    }
...
{
    size_t argc = HUKS_NAPI_DERIVE_KEY_MAX_ARGS;
    napi_value argv[HUKS_NAPI_DERIVE_KEY_MAX_ARGS] = {0};
    NAPI_CALL(env, napi_get_cb_info(env, info, &argc, argv, nullptr, nullptr));

    if (argc < HUKS_NAPI_DERIVE_KEY_MIN_ARGS) {
...
static napi_value DeriveKeyWriteResult(napi_env env, DeriveKeyAsyncContext context)
{
    return GenerateHksResult(env,
        context->result,
        ((context->result == HKS_SUCCESS && context->derivedKey != nullptr) ? context->derivedKey->data : nullptr),
        (context->result == HKS_SUCCESS && context->derivedKey != nullptr) ? context->derivedKey->size : 0);
}

static napi_value DeriveKeyAsyncWork(napi_env env, DeriveKeyAsyncContext context)
{
    ...
    napi_create_async_work(
        ...
        [](napi_env env, void *data) {
            DeriveKeyAsyncContext context = static_cast<DeriveKeyAsyncContext>(data);

            context->derivedKey = (HksBlob *)HksMalloc(sizeof(HksBlob));
            if (context->derivedKey != nullptr) {
                context->derivedKey->data = (uint8_t *)HksMalloc(HKS_MAX_DERIVED_KEY_SIZE);
                context->derivedKey->size = HKS_MAX_DERIVED_KEY_SIZE;
            }

            context->result = HksDeriveKey(context->paramSet, context->keyAlias, context->derivedKey);
        },
        [](napi_env env, napi_status status, void *data) {
            DeriveKeyAsyncContext context = static_cast<DeriveKeyAsyncContext>(data);
            napi_value result = DeriveKeyWriteResult(env, context);
            if (result == nullptr) {
                return;
            }
            if (context->callback != nullptr) {
            ...
            DeleteDeriveKeyAsyncContext(env, context);
        },
        (void *)context,
        &context->asyncWork);
...
{
    DeriveKeyAsyncContext context = CreateDeriveKeyAsyncContext();
    if (context == nullptr) {
        HKS_LOG_E("could not create context");
        return nullptr;
    }

    napi_value result = DeriveKeyParseParams(env, info, context);
    if (result == nullptr) {
    ...
    result = DeriveKeyAsyncWork(env, context);
    if (result == nullptr) {
