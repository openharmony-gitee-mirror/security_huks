...
namespace HuksNapi {
namespace {
constexpr int HUKS_NAPI_ENCRYPT_MIN_ARGS = 2;
constexpr int HUKS_NAPI_ENCRYPT_MAX_ARGS = 3;
...
static napi_value EncryptParseParams(napi_env env, napi_callback_info info, EncryptAsyncContext *context)
{
    ...
    if (argc < HUKS_NAPI_ENCRYPT_MIN_ARGS) {
...
static napi_value EncryptKeyWriteResult(napi_env env, EncryptAsyncContext *context)
{
    return GenerateHksResult(env,
        context->result,
        ((context->result == HKS_SUCCESS && context->cipherText != nullptr) ? context->cipherText->data : nullptr),
    ...
    napi_create_async_work(
        ...
        [](napi_env env, void *data) {
            ...
            if (context->cipherText != NULL) {
                context->cipherText->data =
                    (uint8_t *)HksMalloc(context->plainText->size + HKS_CIPHER_TEXT_DEFALT_PADDING);
                context->cipherText->size = context->plainText->size + HKS_CIPHER_TEXT_DEFALT_PADDING;
            }
        ...
        [](napi_env env, napi_status status, void *data) {
            ...
            DeleteEncryptAsyncContext(env, context);
        },
        (void *)context,
        &context->asyncWork);
...
napi_value HuksNapiEncrypt(napi_env env, napi_callback_info info)
{
    ...
    napi_value result = EncryptParseParams(env, info, context);
    if (result == nullptr) {
    ...
    result = EncryptAsyncWork(env, context);
    if (result == nullptr) {
