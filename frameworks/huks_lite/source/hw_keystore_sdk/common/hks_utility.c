...
static size_t hks_get_clone_str_len(const char *src_str, uint32_t str_len_max)
{
    ...
    if ((str_len == 0) || (str_len > str_len_max)) {
        log_error("invalid src_str len=%u, max_len=%u", str_len, str_len_max);
        return 0;
    }
    ...
    uint32_t u8_buf_len, const uint32_t *u8_buf_offset)
{
    ...
    if ((*u8_buf_offset >= u8_buf_len) ||
        ...
        log_error("invalid para,buf_offset=%u,buf_len=%u", *u8_buf_offset, u8_buf_len);
    ...
    uint32_t *u8_buf_offset)
{
    ...
    for (; i < HKS_U16_BYTE_NUM; i++) {
        uint32_t offset = *u8_buf_offset + g_hks_u16_byte_offset[endian][i];
    ...
    uint32_t *u8_buf_offset, uint16_t *u16_var)
{
    ...
    int32_t rc = hks_chk_u8_buf_para(HKS_U16_BYTE_NUM, u8_buf, u8_buf_len, u8_buf_offset);
...
int32_t hks_u32_2_u8_buf(uint32_t u32_var, uint8_t *u8_buf, uint32_t u8_buf_len,
    uint32_t *u8_buf_offset)
{
    int32_t rc = hks_chk_u8_buf_para(HKS_U32_BYTE_NUM, u8_buf, u8_buf_len, u8_buf_offset);
    ...
    for (; i < HKS_U32_BYTE_NUM; i++) {
        uint32_t offset = *u8_buf_offset + g_hks_u32_byte_offset[endian][i];
...
int32_t hks_u8_2_u8_buf(uint8_t u8_var, uint8_t *u8_buf, uint32_t u8_buf_len,
    uint32_t *u8_buf_offset)
{
    int32_t rc = hks_chk_u8_buf_para(sizeof(uint8_t), u8_buf, u8_buf_len, u8_buf_offset);
    ...
    uint8_t *u8_buf, uint32_t u8_buf_len, uint32_t *u8_buf_offset)
{
    ...
    int32_t rc = hks_chk_u8_buf_para(u8_array_size, u8_buf, u8_buf_len, u8_buf_offset);

    if (rc != HKS_STATUS_OK)
        return rc;

    errno_t err = memcpy_s(u8_buf + *u8_buf_offset, u8_buf_len - *u8_buf_offset, u8_array, u8_array_size);
    ...
    uint32_t *u8_buf_offset, uint8_t *u8_array, uint32_t u8_array_size)
{
    ...
    errno_t err = memcpy_s(u8_array, u8_array_size, u8_buf + *u8_buf_offset, u8_array_size);
    ...
    uint32_t u8_buf_len, uint32_t *u8_buf_offset)
{
    ...
    /* Fill year */
    int32_t rc = hks_u16_2_u8_buf(time_var->year, u8_buf, u8_buf_len, u8_buf_offset);
    ...
    uint32_t *u8_buf_offset, struct hks_time *time_var)
{
    ...
    /* Extract year */
    int32_t rc = hks_u8_buf_2_u16(u8_buf, u8_buf_len, u8_buf_offset, &(time_var->year));

    if (rc != HKS_STATUS_OK)
        return rc;

    /* Extract month */
    rc = hks_u8_buf_2_u8(u8_buf, u8_buf_len, u8_buf_offset, &(time_var->mon));
    if (rc != HKS_STATUS_OK)
        return rc;

    /* Extract day */
    rc = hks_u8_buf_2_u8(u8_buf, u8_buf_len, u8_buf_offset, &(time_var->day));
    if (rc != HKS_STATUS_OK)
        return rc;

    /* Extract hour */
    rc = hks_u8_buf_2_u8(u8_buf, u8_buf_len, u8_buf_offset, &(time_var->hour));
    if (rc != HKS_STATUS_OK)
        return rc;

    /* Extract minuter */
    rc = hks_u8_buf_2_u8(u8_buf, u8_buf_len, u8_buf_offset, &(time_var->min));
    if (rc != HKS_STATUS_OK)
        return rc;

    /* Extract second */
    rc = hks_u8_buf_2_u8(u8_buf, u8_buf_len, u8_buf_offset, &(time_var->sec));
    if (rc != HKS_STATUS_OK)
        return rc;
